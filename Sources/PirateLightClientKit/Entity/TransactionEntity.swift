    ...
    public struct Overview {
        ...
        public enum State {
            ...
            init(
                ...
                if let minedHeight, (currentHeight - minedHeight) >= PirateSDK.defaultStaleTolerance {
                    self = .confirmed
                } else if let minedHeight, (currentHeight - minedHeight) < PirateSDK.defaultStaleTolerance {
                    self = .pending
...
extension ZcashTransaction.Overview {
    ...
    func anchor(network: PirateNetwork) -> BlockHeight? {
        guard let minedHeight = self.minedHeight else { return nil }
        if minedHeight != -1 {
            return max(minedHeight - PirateSDK.defaultStaleTolerance, network.constants.saplingActivationHeight)
        }

        guard let expiryHeight = self.expiryHeight else { return nil }
        if expiryHeight != -1 {
            return max(expiryHeight - PirateSDK.expiryOffset - PirateSDK.defaultStaleTolerance, network.constants.saplingActivationHeight)
        }
